...
export const placeDirectOrder = async (req, res) => {
  try {
    const userId = req.user._id;
    const { productId, quantity, shippingInfo, totalAmount } = req.body;
...
export const getUserOrders = async (req, res) => {
  try {
    ...
    const { orders, total, totalPages } = await findUserOrders(req.user._id, {
      page,
      limit,
      status,
    });
...
export const getOrderDetails = async (req, res) => {
  try {
    const { id } = req.params;
    const { error, order } = await findOrderById(
      id,
      req.user._id,
      req.user.is_admin,
    );
    ...
    res
      .status(500)
      .json({ message: "Failed to fetch order details", error: err.message });
  }
};
